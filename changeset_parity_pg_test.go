...
// TestChangeSetsParityPostgres re-runs the change-set suite
// (infrastructure/memory/changeset_test.go) against Postgres, so the
// draft-overlay preview, the two-person approval gate, and — most
// importantly — the all-or-nothing PUBLISH transaction run against the real
// SQL store. It is the second top-priority port of issue #207.
func TestChangeSetsParityPostgres(t *testing.T) {
	pool := openTestDB(t)
	...
	Convey("Given a product type with name and price (Postgres)", t, func() {
		truncateAll(t, pool)
