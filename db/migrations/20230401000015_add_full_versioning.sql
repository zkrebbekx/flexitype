...
CREATE TABLE flexitype.attribute_definition_version (
    ...
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (type_id, type_version, attribute_name),
    FOREIGN KEY (type_id, type_version) REFERENCES flexitype.type_definition_version(type_id, version)
);
...
CREATE TABLE flexitype.validation_rule_version (
    ...
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (type_id, type_version, attribute_name, rule_type, sort_order),
    FOREIGN KEY (type_id, type_version, attribute_name) 
        REFERENCES flexitype.attribute_definition_version(type_id, type_version, attribute_name)
);
...
CREATE TABLE flexitype.attribute_cascade_version (
    ...
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (type_id, type_version, attribute_name, cascade_id),
    FOREIGN KEY (type_id, type_version, attribute_name) 
        REFERENCES flexitype.attribute_definition_version(type_id, type_version, attribute_name)
);
...
ALTER TABLE flexitype.attribute_value
ADD CONSTRAINT uq_attr_value_instance_version_name_idx 
UNIQUE (instance_id, instance_version, attribute_name, list_index);
...
ON flexitype.attribute_value(instance_id, instance_version, attribute_name);

-- Create index for type definition versions
CREATE INDEX idx_type_definition_version 
ON flexitype.type_definition_version(type_id, version DESC);

-- Create indexes for attribute definition versions
CREATE INDEX idx_attribute_definition_version_name 
ON flexitype.attribute_definition_version(attribute_name);
...
ON flexitype.validation_rule_version(type_id, type_version, attribute_name);

-- Create indexes for attribute cascade versions
CREATE INDEX idx_attribute_cascade_version_query 
ON flexitype.attribute_cascade_version(type_id, type_version, attribute_name);
