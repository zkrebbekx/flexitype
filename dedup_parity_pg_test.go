...
// TestDuplicateDetectionParityPostgres re-runs the dedup suite
// (infrastructure/memory/dedup_test.go) against Postgres, exercising the
// match-rule/dismissal SQL store and the value fetch that feeds the (Go-side,
// backend-independent) trigram scan: near-duplicate detection, idempotent
// re-scan, dismissal persistence, exact-strategy sensitivity, and content-free
// values.
func TestDuplicateDetectionParityPostgres(t *testing.T) {
	pool := openTestDB(t)
	...
	Convey("Given products whose names nearly match (Postgres)", t, func() {
		truncateAll(t, pool)
