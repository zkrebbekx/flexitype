...
func main() {
	...
	statusAttr := sdk.NewAttribute("status", "Product status", sdk.StringType, true)
	...
	discountAttr := sdk.NewAttribute("discount", "Discount percentage", sdk.FloatType, false)
	discountAttr.AddCascade("price-discount", true, core.CascadeInherit, "price > 1000 => discount = 5.0", 100)

	availableAttr := sdk.NewAttribute("available", "Product is available", sdk.BooleanType, true)
	...
	err = client.AddAttribute(ctx, productType.Name, statusAttr)
	if err != nil {
		log.Fatalf("Failed to add status attribute: %v", err)
	}

	err = client.AddAttribute(ctx, productType.Name, discountAttr)
	if err != nil {
		log.Fatalf("Failed to add discount attribute: %v", err)
	}

	err = client.AddAttribute(ctx, productType.Name, availableAttr)
	if err != nil {
	...
	_, err = client.SetAttributeDisabledState(ctx, productType.Name, statusAttr.Name, true)
	if err != nil {
	...
	productType2, err := client.SaveType(ctx, "Product2", "Product record 2")
	if err != nil {
		log.Fatalf("Failed to create product type 2: %v", err)
	}

	// Add basic attributes that will form a circular dependency
	price2Attr := sdk.NewAttribute("price", "Product price", sdk.FloatType, true)
	price2Attr.AddValidationRule(&sdk.RequiredRule{})

	status2Attr := sdk.NewAttribute("status", "Product status", sdk.StringType, true)
	...
	available2Attr := sdk.NewAttribute("available", "Product is available", sdk.BooleanType, true)
	available2Attr.SetDefaultValue(true)

	// Add first two attributes (these should succeed)
	err = client.AddAttribute(ctx, productType2.Name, price2Attr)
	if err != nil {
		log.Fatalf("Failed to add price2 attribute: %v", err)
	}

	err = client.AddAttribute(ctx, productType2.Name, status2Attr)
	if err != nil {
	...
	err = client.AddAttribute(ctx, productType2.Name, available2Attr)
	if err != nil {
		fmt.Printf("Expected error: %v\n", err)
	} else {
	...
	productType3, err := client.SaveType(ctx, "Product3", "Product record 3")
	if err != nil {
		log.Fatalf("Failed to create product type 3: %v", err)
	}

	// Create an attribute that references a non-existent attribute
	stockAttr := sdk.NewAttribute("stock", "Product stock level", sdk.IntType, true)
	...
	err = client.AddAttribute(ctx, productType3.Name, stockAttr)
	if err != nil {
		fmt.Printf("Expected error: %v\n", err)
	} else {
	...
	// Create a fresh product type for this test
	productType4, err := client.SaveType(ctx, "Product4", "Product record 4")
	if err != nil {
		log.Fatalf("Failed to create product type 4: %v", err)
	}

	// Add base attributes that others will reference
	price4Attr := sdk.NewAttribute("price", "Product price", sdk.FloatType, true)
	err = client.AddAttribute(ctx, productType4.Name, price4Attr)
	if err != nil {
		log.Fatalf("Failed to add price4 attribute: %v", err)
	}

	discount4Attr := sdk.NewAttribute("discount", "Discount percentage", sdk.FloatType, false)
	err = client.AddAttribute(ctx, productType4.Name, discount4Attr)
	if err != nil {
	...
	taxRateAttr := sdk.NewAttribute("taxRate", "Tax rate percentage", sdk.FloatType, true)
	...
	shippingAttr := sdk.NewAttribute("shipping", "Shipping cost", sdk.FloatType, true)
	...
	totalAttr := sdk.NewAttribute("total", "Total price", sdk.FloatType, true)
	...
	err = client.AddAttribute(ctx, productType4.Name, categoryAttr)
	if err != nil {
		log.Fatalf("Failed to add category attribute: %v", err)
	}

	err = client.AddAttribute(ctx, productType4.Name, taxRateAttr)
	if err != nil {
		log.Fatalf("Failed to add tax rate attribute: %v", err)
	}

	err = client.AddAttribute(ctx, productType4.Name, shippingAttr)
	if err != nil {
		log.Fatalf("Failed to add shipping attribute: %v", err)
	}

	err = client.AddAttribute(ctx, productType4.Name, totalAttr)
	if err != nil {
	...
	err = client.DeleteAttribute(ctx, productType4.Name, priceAttr.Name)
	if err != nil {
		fmt.Printf("Expected error: %v\n", err)
	} else {
