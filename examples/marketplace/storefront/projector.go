...
//
// One credential reads one tenant, because the tenant comes from the token.
// That is the property the whole design rests on, and this service works
// WITH it rather than around it: an instance of this storefront serves ONE
// merchant, so in practice this cache holds exactly one client.
//
// An earlier version of this example ran one storefront across every
// merchant, which forced it to hold every merchant's credential — and then a
// cross-tenant scope to avoid that. Both were the wrong shape: a storefront
// per merchant needs no privilege that crosses a tenant boundary at all.
