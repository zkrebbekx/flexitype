...
// TestGraphQLWindowedRelationshipConnections proves the per-parent windowed
// resolution of nested relationship connections (issue #203): a first:N nested
// connection over several parents returns exactly N edges per parent with the
// correct hasNextPage/totalCount, resolves each relationship FIELD against only
// its own definition (so two fields in one query do not cross-contaminate or
// re-run a broad all-links scan), and resumes correctly from a nested cursor.
func TestGraphQLWindowedRelationshipConnections(t *testing.T) {
	Convey("Given products, suppliers and two product→supplier relationships with a fan-out", t, func() {
		...
		// productsByID indexes the top-level connection's product nodes by id.
