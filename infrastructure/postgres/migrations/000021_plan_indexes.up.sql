...
--
-- Each build drops an INVALID namesake first, because a failed concurrent
-- build leaves one behind that IF NOT EXISTS would then skip forever.
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
                WHERE c.relname = 'idx_flexitype_attribute_value_entity_all' AND NOT i.indisvalid) THEN
        EXECUTE 'DROP INDEX idx_flexitype_attribute_value_entity_all';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
                WHERE c.relname = 'idx_flexitype_attribute_value_media_key' AND NOT i.indisvalid) THEN
        EXECUTE 'DROP INDEX idx_flexitype_attribute_value_media_key';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
                WHERE c.relname = 'idx_flexitype_attribute_value_entity_attr' AND NOT i.indisvalid) THEN
        EXECUTE 'DROP INDEX idx_flexitype_attribute_value_entity_attr';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
                WHERE c.relname = 'idx_flexitype_webhook_delivery_envelope' AND NOT i.indisvalid) THEN
        EXECUTE 'DROP INDEX idx_flexitype_webhook_delivery_envelope';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
                WHERE c.relname = 'idx_flexitype_webhook_subscription_active' AND NOT i.indisvalid) THEN
        EXECUTE 'DROP INDEX idx_flexitype_webhook_subscription_active';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
        ...
        EXECUTE 'DROP INDEX IF EXISTS idx_flexitype_attribute_value_trgm';
        EXECUTE 'DROP INDEX IF EXISTS idx_flexitype_attribute_value_trgm_lower';
    END IF;
END $$;
