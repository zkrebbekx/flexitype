...
// TestScopedValuesParityPostgres re-runs the scoped-value suite
// (infrastructure/memory/scoped_values_test.go) against Postgres: independent
// read-back per locale/channel, rejection of a scope on a non-scopable
// attribute, scope-restricted FQL filtering, and per-scope uniqueness — all
// resting on the (entity, attribute, locale, channel) value identity in SQL.
func TestScopedValuesParityPostgres(t *testing.T) {
	pool := openTestDB(t)
	...
	Convey("Given a localizable+scopable description across locale × channel (Postgres)", t, func() {
		truncateAll(t, pool)
