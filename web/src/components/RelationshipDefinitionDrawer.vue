...
import type { TypeDefinition, VersionPolicy } from '@/lib/api'
...
const baseOptions = computed(() => [
  ...
  ...(bases.data.value?.items ?? [])
    .filter((d) => d.parent_type_id === form.parent_type_id && d.child_type_id === form.child_type_id)
...
<template>
  <Drawer
    ...
    subtitle="A named link between a parent type and a child type. It carries its own attributes, editable per link."
    @close="emit('close')"
  >
    <form class="flex flex-col gap-4" @submit.prevent="save.mutate()">
      ...
      <div class="grid grid-cols-[1fr_auto_1fr] items-end gap-2">
        <Select v-model="form.parent_type_id" label="Parent type" :options="typeOptions" />
        <ArrowRight :size="16" class="mb-2.5 text-(--text-muted)" />
        <Select v-model="form.child_type_id" label="Child type" :options="typeOptions" />
      </div>
